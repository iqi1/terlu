...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/modules/tlContract/audit?id=${tlContract.id}">请假申请审核</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="tlContract" action="${ctx}/modules/tlContract/audit" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table id="" border="0" cellpadding="1" cellspacing="0" class="table table-striped table-bordered table-condensed"
			...
			<tr>
			<td>
			<c:if test="${empty tlContract.procInsId}"> 
			<input id="btnSubmit" class="btn btn-primary" type="submit" value="提交审核" onclick="$('#flag').val(2)"/>&nbsp;
			</c:if>
			<c:if test="${not empty tlContract.procName && tlContract.status == 3}"> 

			<shiro:hasPermission name="modules:tlContract:audit">
			<input id="btnSubmit" class="btn btn-primary" type="submit" value="同 意" onclick="$('#flag').val(1)"/>&nbsp;
			
			<c:if test="${tlContract.procName != '客户科主任'}"> 
	...
	<act:histoicFlow procInsId="${tlContract.procInsId}"/>
	</form:form>
	<script type="text/javascript">
	$(function() {
		$("#tabs").tabs();
