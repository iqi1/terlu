...
J.Page = (function($){
    ...
    var _formatHashurl = function(hash){
    	var url = hash.indexOf('#') == 0 ? hash.substr(1) : hash;
    ...
    var loadSectionTpl = function(hash,callback){
        ...
        var id = _formatHashurl(hash);
        //当前dom中不存在，需要从服务端加载
        var url = J.settings.remotePage[hash];
        //检查remotePage中是否有配置,没有则自动从basePagePath中装载模板
        url || (url = ctx + "/" + id);
        console.log(url);
        //(url = J.settings.basePagePath+id+J.settings.basePageSuffix);
